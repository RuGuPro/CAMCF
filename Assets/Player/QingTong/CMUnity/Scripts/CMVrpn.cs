...
public static class CMVrpn
{
    ...
    [DllImport("CMVrpn")]
    private static extern void CMVrpnQuitExtern();

    [DllImport("CMVrpn")]
    private static extern void CMVrpnEnableLog(bool IsEnableTrackLog);
    ...
    [DllImport("CMVrpn")]
    public static extern double CMTrackerNoPredictExtern(string address, int channel, int component);
    ...
    [DllImport("CMVrpn")]
    public static extern double CMTrackerExternTC(string address, int channel, ref int timecode, [In, Out] double[] position, [In, Out] double[] quaternion);
    ...
    public delegate void UpdateHumanMapCallback(IntPtr CallBackFun_agrs, VrpnHumanMap CurHumanMap);
    [DllImport("CMVrpn", CallingConvention = CallingConvention.StdCall)]
    ...
    {
        CMVrpnEnableLog(IsEnableTrackLog);
    }
    ...
    {
        return new Vector3(
        (float)CMTrackerNoPredictExtern(address, channel, 0) / 1000f,
        (float)CMTrackerNoPredictExtern(address, channel, 2) / 1000f,
        (float)CMTrackerNoPredictExtern(address, channel, 1) / 1000f);
    }
    ...
    public static Quaternion CMQuat(string address, int channel, bool lockYRotation = false)
    {
        ...
        (float)CMTrackerNoPredictExtern(address, channel, 3),
        (float)CMTrackerNoPredictExtern(address, channel, 5),
        (float)CMTrackerNoPredictExtern(address, channel, 4),
        -(float)CMTrackerNoPredictExtern(address, channel, 6));
    }
    ...
    /// </summary>
    /// <param name="address">ServerIP:"MCAvatar@192.168.3.178"</param>
